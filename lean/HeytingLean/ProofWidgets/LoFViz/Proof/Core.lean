...
partial def buildTermGraph (fuel depth : Nat) : Expr → TermBuildM NodeId
  | e => do
      if fuel = 0 then
        pushNodeM
          { kind := .term
...
partial def visitExpr (root : Name) (fuel : Nat) :
    Expr → StateM (Array Primitive) Unit
  | e =>
      if fuel = 0 then
        pure ()
      else
        ...
        | Expr.app f x => do
            ...
            visitExpr root (fuel - 1) f
            visitExpr root (fuel - 1) x
        | Expr.letE _ _ value body _ => do
            visitExpr root (fuel - 1) value
            visitExpr root (fuel - 1) (body.instantiate1 value)
        | Expr.mdata _ inner =>
            visitExpr root (fuel - 1) inner
        | Expr.proj _ _ inner =>
            visitExpr root (fuel - 1) inner
        | _ => pure ()
